...
- [Server Side](#server-side)
  ...
  - [session vs token](#session-vs-token)
    - [authentication](#authentication)
- [Database](#database)
  - [Aggregate and Scalar Functions](#aggregate-and-scalar-functions)
...
* cokkie is work as a seesion Id.
* session id is save in the backend database.
* every time client send request with the cockie, backend server compare it with the session id, to identify the client, to be able to to keep the session.
![](session-cookie.png)
* drawbacks, because the session id is saved in the database
* right backend server and database are running on multiple server, so the syncrazaiton between servers are problems.
...
#### jwt
* jwt is generated by the server, and sent to the client.
* jwt is never stored on the server, and is only sent to the client.
* is base on the private key stored in the server.
* client can send the jwt in the header with bearer token.
* there no need for database lookup.

#### session vs token
##### authentication
* session the authentication is done on the server side
* token the authentication is done on the client side
...
https://www.geeksforgeeks.org/establishing-jdbc-connection-in-java/amp/
...
#### JPA/Hibernate/JDBC  
* SPRING DATA JPA
  * no-code implementation of the repository pattern
  * creation of database queries from methods names
  * Spring data jpa remove the boiler plate code of the jpa.
  * make the data access layer more easy to use and maintain.
  * reason to use spring data jpa
    * No-code Respository
    * reduce boilerplate code(no read and write code)
    * generation queries based on method names
  
* JPA: Java Persistence API
  * specification for object-relational mapping (ORM)
  * Reduce the complexity of the persistence layer by providing a standard API for accessing and manipulating data.
  * object relational mapping
  * relational database
  * class(Entity) => table
  * object => row or row in the join table
  
* Hibernate: 
  * hibernate is the implimentation of jpa
  * Hibernate is a framework for mapping database tables to Java classes and vice versa.
...
![](spring-spring-boot-feature.png)
![](/spring-spring-boot-relation.png)
![](spring-vs-spring-boot.png)
* spring boot is abstraction of spring framework

1.  Describe hibernate ORM
2.  Asked what dependency injection is
3.  Asked about SQL & general experience with it
4.  Difference between collections & streams
5.  Difference between Linked List & Array List
6.  Can entity classes be marked as final?
7.  Is “finally” guaranteed to run?
...
9.  Asked about RESTful API
10. Asked about what kind of exceptions I can expect to see thrown when comparing strings 
...
the final keyword (how do we make a class immutable)
exceptions and how they're handled
REST API
[words describing what http is, asked what it is called]
http methods
how do you set up a spring boot application (which im realizing i forgot to mention the @SpringBootApplication annotation lol)
since I told them im devops lead, they asked me what the AWS cloud services are and told me to list them off
RDS, S3, EC2, CodeBuild, CodePipeline
and asked if i knew what cloud watch is

- Tel me about yourself
- Features of spring boot
- Spring boot annotations
- Type of dependency injection in Spring boot
